...
def refresh_seed(seed_string: int | str | None) -> int:
    ...
    if (
        seed_value < constants.MIN_SEED
        or seed_value > constants.MAX_SEED
        or seed_string == -1
    ):
        return RANDOM_SEED
...
def check_models_exist(file_name: str, model_type: str) -> str:
    ...
    if file_name not in (config.model_filenames + config.lora_filenames):
        logger.std_warn(
            f"[Warning] Wrong {model_type} model input: '{file_name}' not found, falling back to default '{default_base_model_name if model_type == 'base' else default_refiner_model_name}'"
        )
        if model_type == "base":
        ...
        return "None"
    logger.info(f"Using {model_type} model: {file_name}")
    return file_name
...
def req_to_params(req: Text2ImgRequest) -> ImageGenerationParams:
    ...
    loras = [
        (lora.enabled, check_models_exist(lora.model_name, "lora"), lora.weight)
    ...
    uov_method = (
        flags.disabled
        if not isinstance(req, (ImgUpscaleOrVaryRequest, ImgUpscaleOrVaryRequestJson))
        else req.uov_method.value
    )
    upscale_value = (
        None
        if not isinstance(req, (ImgUpscaleOrVaryRequest, ImgUpscaleOrVaryRequestJson))
        else req.upscale_value
    )
    outpaint_selections = (
        []
        if not isinstance(
            req, (ImgInpaintOrOutpaintRequest, ImgInpaintOrOutpaintRequestJson)
        )
        else [s.value for s in req.outpaint_selections]
    )
    outpaint_distance_left = (
        ...
        else req.outpaint_distance_left
    )
    outpaint_distance_right = (
        ...
        else req.outpaint_distance_right
    )
    outpaint_distance_top = (
        ...
        else req.outpaint_distance_top
    )
    outpaint_distance_bottom = (
        ...
        else req.outpaint_distance_bottom
    )
    ...
    if (
        isinstance(req, (ImgInpaintOrOutpaintRequest, ImgInpaintOrOutpaintRequestJson))
        and req.input_image is not None
    ):
        ...
        inpaint_input_image = {"image": input_image, "mask": input_mask}
    ...
    if isinstance(
        ...
        if len(req.image_prompts) > 0 and uov_input_image is not None:
            ...
            len(req.image_prompts) > 0
            and not isinstance(req, Text2ImgRequestWithPrompt)
            and req.input_image is not None
        ):
        ...
        for img_prompt in req.image_prompts:
            if img_prompt.cn_img is not None:
                ...
                if img_prompt.cn_stop is None or img_prompt.cn_stop == 0:
                    img_prompt.cn_stop = flags.default_parameters[
                        img_prompt.cn_type.value
                    ][0]
                if img_prompt.cn_weight is None or img_prompt.cn_weight == 0:
                    img_prompt.cn_weight = flags.default_parameters[
                        img_prompt.cn_type.value
                    ][1]
    ...
    if len(image_prompts) < config.default_controlnet_image_count:
        ...
        image_prompts += [dp] * (
            config.default_controlnet_image_count - len(image_prompts)
        )
    ...
    if req.advanced_params is not None:
        ...
        if adp.refiner_swap_method not in ["joint", "separate", "vae"]:
            print(
                f"[Warning] Wrong refiner_swap_method input: {adp.refiner_swap_method}, using default"
            )
            adp.refiner_swap_method = "joint"

        if adp.sampler_name not in flags.sampler_list:
            print(
                f"[Warning] Wrong sampler_name input: {adp.sampler_name}, using default"
            )
            adp.sampler_name = default_sampler

        if adp.scheduler_name not in flags.scheduler_list:
            print(
                f"[Warning] Wrong scheduler_name input: {adp.scheduler_name}, using default"
            )
            adp.scheduler_name = default_scheduler

        if adp.inpaint_engine not in flags.inpaint_engine_versions:
            print(
                f"[Warning] Wrong inpaint_engine input: {adp.inpaint_engine}, using default"
            )
...
def generate_async_output(
    ...
    if task.is_finished:
        if task.finish_with_error:
            ...
            job_result = generate_image_result_output(
                task.task_result, task.req_param.require_base64
            )
